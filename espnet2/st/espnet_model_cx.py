...
class ESPnetSTModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if text_prev is not None:
            ...
            ), (speech.shape, speech_lengths.shape, text.shape, text_lengths.shape,text_prev.shape,text_prev_lengths.shape)
        else:
            assert (
                speech.shape[0]
                == speech_lengths.shape[0]
    ...
    def _calc_mt_att_loss(
        ...
        ys_prev_pad: Optional[torch.Tensor] = None,
        ys_prev_lens: Optional[torch.Tensor] = None,
        st: bool = True,
    ):
        if ys_prev_pad is not None:
            ys = [y[y != self.tgt_ignore_id] for y in ys_pad]
            ys_prev = [y[y != self.tgt_ignore_id] for y in ys_prev_pad]
            _eos_sos = ys_pad.new([self.sos])
            ...
            for y_prev, y in zip(ys_prev, ys):
                if self.na in y_prev:
                    ...
                    y_in = [_sop, y_prev, _eos_sos, y]
                    y_in_len = len(y_prev) + len(y) + 2
                    y_out = [self.tgt_ignore_id * ys_pad.new_ones(len(y_prev) + 1), y, _eos_sos]
