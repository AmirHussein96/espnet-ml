...
if [[ $src_lang == 'ara' ]]; then
    scale_data_base="/exp/scale23/users/ahussein/arabic_normalized/3-way"
else
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    opts="--fs ${fs_str} --min-duration $min_duration --context $context --max-sep $max_sep "
    if "${stereo}"; then
        opts+="--stereo "
    fi
    ...
    for set in $train_set $dev_set; do
        ...
        output_dir=$(pwd)/data/${set}_${src_lang}
        ...
        cp $output_dir/tmp_tgt/text $output_dir/text.eng
		cp $output_dir/tmp_tgt/text.prev $output_dir/text.prev.eng
		cp $output_dir/tmp_tgt/text.prev2 $output_dir/text.prev2.eng
		cp $output_dir/tmp_tgt/text.prev3 $output_dir/text.prev3.eng

        # Copy source text to include tc.${src_lang}
        cp $output_dir/text $output_dir/text.${src_lang}
    ...
    for testset in $cts_testlist; do
        ...
        output_dir=$(pwd)/data/${testset}_test_${src_lang}
    ...
    if [ ! -z $ood_testset ]; then
        ...
        output_dir=$(pwd)/data/${ood_testset}_test_${src_lang}
