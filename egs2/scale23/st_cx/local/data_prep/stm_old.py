...
class Stm(object):
    ...
    def espnet_lists(self, stereo=False, ignore_segments=False, fs=8000, context=1, max_sep=10):
        ...
        for idx, s in enumerate(self._utts):
            ...
            prev_ = [PAREN_RE.sub('\1', self._utts[u].transcript) + ' xuttx ' if u != max_prev[-1] else PAREN_RE.sub('\1', self._utts[u].transcript) for u in max_prev]
    ...
    def write_espnet_dir(self, data_dir, stereo=False, ignore_segments=False, fs=8000, context=1, max_sep=30):
        ...
        text, text_prev, wav_scp, spk2utt, reco2channel, segments = self.espnet_lists(stereo=stereo, ignore_segments=ignore_segments, fs=fs, context=context, max_sep=max_sep)
