...
class Stm(object):
    ...
    def espnet_lists(self, stereo=False, ignore_segments=False, fs=8000, context=1, max_sep=10):
        """
        Creates wav_scp, segments (optional), text, and spk2utt list/dicts
            context: previous context in terms of # of utterances
            max_sep: maximum separation between consecutive segments
        """
        wav_scp = set()
        segments = set()
        text = []
        ...
        context = context
        for idx, s in enumerate(self._utts):
            ...
            max_prev = self.find_max_prev(idx, max_sep, context)
            prev_ = []

            for u in max_prev:
                if u != max_prev[-1]:
                    prev_tmp = PAREN_RE.sub('\1', self._utts[u].transcript) + ' xuttx '
                    prev_tmp = prev_tmp.split()
                    if len(prev_tmp) > 50:
                        prev_.append(" ".join(prev_tmp[-50:]))
                    else:
                        prev_.append(" ".join(prev_tmp))
                else:
                    prev_.append(PAREN_RE.sub('\1', self._utts[u].transcript))

            #prev_ = [PAREN_RE.sub('\1', self._utts[u].transcript) + ' <utt> ' if u != max_prev[-1] else PAREN_RE.sub('\1', self._utts[u].transcript) for u in max_prev]
            #prev_ = [PAREN_RE.sub('\1', self._utts[u].transcript) for u in max_prev]
            text_prev.append('%s %s' % (utt_id, " ".join(prev_) if prev_ else "xnax"))

        return text, text_prev, wav_scp, spk2utt, reco2channel, segments
    ...
    # def wav2utts_(self):
    #     "Creates wav to segments dictionary"
    #     wav2utts = {}
    #     for idx, utt in enumerate(self._utts):
    #         seg_id, wav_id, s_t, e_t = seg.split()  # e.g.: 'ted_767.wav'
    #         if wav_id not in wav2utts:
    #             wav2utts[wav_id] = []
    #         utt['seg_id'] = seg_id
    #         utt['s_time'] = s_t
    #         utt['e_time'] = e_t
    #         wav2utts[wav_id].append(utt)
    #     return wav2utts

    def find_max_prev(self, pos, max_sep, context):
        # ..., pos-2, pos-1 (not including pos)
        results = []
        rec_id = self._utts[pos].recording_id(stereo=False)
        if pos > 0:
            prev_rec_id = self._utts[pos-1].recording_id(stereo=False)
            #while pos - 1 >= 0 and len(results) < context and (self._utts[pos].start_time - self._utts[pos-1].start_time) <= max_sep and rec_id == prev_rec_id:
            while pos - 1 >= 0 and len(results) < context and rec_id == prev_rec_id:
                results.append(pos - 1)
                pos -= 1
                prev_rec_id = self._utts[pos-1].recording_id(stereo=False)
        return results[::-1]

    def write_espnet_dir(self, data_dir, stereo=False, ignore_segments=False, fs=8000, context=1, max_sep=30):
        """
        Creates an espnet datadir for this stm
        """
        # text (uttid <transcription>)
        # text.prev (uttid <transcription>) # text with previous context 
        # wav.scp (uttid path) (use recid if segments)
        # utt2spk (uttid speakerid) (use uttid if no speaker)
        # segments (uttid recid start end)
        # 

        text, text_prev, wav_scp, spk2utt, reco2channel, segments = self.espnet_lists(stereo=stereo, ignore_segments=ignore_segments, fs=fs, context=1, max_sep=max_sep)
        _, text_prev2, _, _, _, _ = self.espnet_lists(stereo=stereo, ignore_segments=ignore_segments, fs=fs, context=2, max_sep=max_sep)
        _, text_prev3, _, _, _, _ = self.espnet_lists(stereo=stereo, ignore_segments=ignore_segments, fs=fs, context=3, max_sep=max_sep)
        ...
        with open(os.path.join(data_dir, 'text.prev'), 'w', encoding='utf-8') as fh:
            Stm._write_sorted_list(fh, text_prev)

        with open(os.path.join(data_dir, 'text.prev2'), 'w', encoding='utf-8') as fh:
            Stm._write_sorted_list(fh, text_prev2)
            
        with open(os.path.join(data_dir, 'text.prev3'), 'w', encoding='utf-8') as fh:
            Stm._write_sorted_list(fh, text_prev3)

        with open(os.path.join(data_dir, 'stm'), 'w', encoding='utf-8') as fh:
            self.write(fh)


    def filter(self, min_utt_dur=0.1, max_utt_dur=30.):
        s = Stm([])
