...
def _get_parser():
    ...
    parser.add_argument("--context", default=1, help="Previous context in terms of utterances")
    parser.add_argument("--max-sep", default=30, help="Maximum seperation between consecutive utterances in seconds")
...
def stm_to_espnet(
    ...
    stm.write_espnet_dir(output_dir, stereo=use_stereo, ignore_segments=ignore_segments, fs=fs, context=context, max_sep=max_sep)
...
def main(args):
    ...
    stm_to_espnet(input_stm, output_dir, use_stereo, ignore_segments, min_dur=min_dur, fs=fs, start_at_zero=start_at_zero, context=context, max_sep=max_sep)
