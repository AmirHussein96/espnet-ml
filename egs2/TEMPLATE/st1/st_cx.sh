...
if [ $use_src_lang = true ]; then
    utt_extra_files="text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang} text.prev.${tgt_case}.${tgt_lang}"
else
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        # shellcheck disable=SC2086
        if "${context}"; then
            log "Adding context to validation"
            _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
            ...
            if ! "${context}"; then
                ...
                ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/st_inference.JOB.log \
                    ${python} -m ${st_inference_tool} \
                        ...
                        --key_file "${_logdir}"/keys.JOB.scp \
                        --data_path_and_name_and_type "${_data}/${_scp},speech,${_type}" \
                        --data_path_and_name_and_type "${_data}/text.prev.${tgt_case}.${tgt_lang},text_prev,text" \
                        --st_train_config "${st_exp}"/config.yaml \
                        --st_model_file "${st_exp}"/"${inference_st_model}" \
                        --output_dir "${_logdir}"/output.JOB \
                        ${_opts} ${inference_args} || { cat $(grep -l -i error "${_logdir}"/st_inference.*.log) ; exit 1; }
