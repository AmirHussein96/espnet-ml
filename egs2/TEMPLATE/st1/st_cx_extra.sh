...
prev_context_tgt_type=  # type of previous context 
context=false
text_prev_apply_prob=0
domain=
...
if [ $use_src_lang = true ]; then
    utt_extra_files="text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang} text.prev.${tgt_case}.${tgt_lang} text.prev2.${tgt_case}.${tgt_lang} text.prev3.${tgt_case}.${tgt_lang} text.prev.rand.${tgt_case}.${tgt_lang}"
else
...
if [ -n "${domain}" ]; then
	token_listdir=data/${domain}_${src_lang}_${tgt_lang}_token_list
	echo "token_list ${token_listdir}"
else
...
if [ -z "${st_tag}" ]; then
	...
	if (( $(echo "$text_prev_apply_prob > 0.0" |bc -l) ));then
		st_tag+="_text_prev_apply_prob${text_prev_apply_prob}"
	fi
fi
if [ -z "${lm_tag}" ]; then
    if [ -n "${lm_config}" ]; then
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ ${prev_context_tgt_type} = "rand" ]; then
            log "Using random context"
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
            _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
        
        elif [ ${prev_context_tgt_type} = "context2" ]; then
            log "Using  context size 2"
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
            _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
        elif [ ${prev_context_tgt_type} = "context3" ]; then
            log "Using  context size 3"  
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
            _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
        else
            log "Using previous context size 1"
            _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
            _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.stcx_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${tgt_bpemodel}" \
                --token_type "${tgt_token_type}" \
        ...
        if "${context}"; then
            if [ ${prev_context_tgt_type} = "rand" ]; then
                <"${st_stats_dir}/train/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/train/text_prev_rand_shape.${tgt_token_type}"

                <"${st_stats_dir}/valid/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/valid/text_prev_rand_shape.${tgt_token_type}"
            
            elif [ ${prev_context_tgt_type} = "context2" ]; then
                <"${st_stats_dir}/train/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/train/text_prev2_shape.${tgt_token_type}"

                <"${st_stats_dir}/valid/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/valid/text_prev2_shape.${tgt_token_type}"

            elif [ ${prev_context_tgt_type} = "context3" ]; then
                <"${st_stats_dir}/train/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/train/text_prev3_shape.${tgt_token_type}"

                <"${st_stats_dir}/valid/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/valid/text_prev3_shape.${tgt_token_type}"

            else
                <"${st_stats_dir}/train/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/train/text_prev_shape.${tgt_token_type}"

                <"${st_stats_dir}/valid/text_prev_shape" \
                    awk -v N="$(<${tgt_token_list} wc -l)" '{ print $0 "," N }' \
                    >"${st_stats_dir}/valid/text_prev_shape.${tgt_token_type}"
            fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        # adding the context to the training
        if "${context}"; then
            if [ ${prev_context_tgt_type} = "rand" ]; then
                log "Adding random context to training in splits"

                _num_splits_opts+="${_st_train_dir}/text.prev.rand.${tgt_case}.${tgt_lang} "
                _num_splits_opts+="${st_stats_dir}/train/text_prev_rand_shape.${tgt_token_type} "

            elif [ ${prev_context_tgt_type} = "context2" ]; then
                log "Adding context 2 to training in splits"

                _num_splits_opts+="${_st_train_dir}/text.prev2.${tgt_case}.${tgt_lang} "
                _num_splits_opts+="${st_stats_dir}/train/text_prev2_shape.${tgt_token_type} "

            elif [ ${prev_context_tgt_type} = "context3" ]; then
                log "Adding context 3 to training in splits"

                _num_splits_opts+="${_st_train_dir}/text.prev3.${tgt_case}.${tgt_lang} "
                _num_splits_opts+="${st_stats_dir}/train/text_prev3_shape.${tgt_token_type} "            
            
            
            else
                log "Adding context to training in splits" 
                _num_splits_opts+="${_st_train_dir}/text.prev.${tgt_case}.${tgt_lang} "
                _num_splits_opts+="${st_stats_dir}/train/text_prev_shape.${tgt_token_type} "
            fi
        fi
        if [ "${num_splits_st}" -gt 1 ]; then
            ...
            if "${context}"; then
                ...
                if [ ${prev_context_tgt_type} = "rand" ]; then
                    log "Adding random context to training"
                    _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${_split_dir}/text_prev_rand_shape.${tgt_token_type} "
                
                elif [ ${prev_context_tgt_type} = "context2" ]; then
                    log "Adding context 2 to training"
                    _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${_split_dir}/text_prev2_shape.${tgt_token_type} "                
                
                elif [ ${prev_context_tgt_type} = "context3" ]; then
                    log "Adding context 3 to training"
                    _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${_split_dir}/text_prev3_shape.${tgt_token_type} "                  
                
                else
                    log "Adding previous context to training"
                    _opts+="--train_data_path_and_name_and_type ${_split_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${_split_dir}/text_prev_shape.${tgt_token_type} "
                fi
            fi
        else
            ...
            if "${context}"; then
                if [ ${prev_context_tgt_type} = "rand" ]; then
                    log "Adding random context to training"
                    _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${st_stats_dir}/train/text_prev_rand_shape.${tgt_token_type} "

                elif [ ${prev_context_tgt_type} = "context2" ]; then
                    log "Adding context 2 to training"
                    _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${st_stats_dir}/train/text_prev2_shape.${tgt_token_type} "
                elif [ ${prev_context_tgt_type} = "context3" ]; then
                    log "Adding context 3 to training"
                    _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${st_stats_dir}/train/text_prev3_shape.${tgt_token_type} "

                else
                    log "Adding context to training"
                    _opts+="--train_data_path_and_name_and_type ${_st_train_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
                    _opts+="--train_shape_file ${st_stats_dir}/train/text_prev_shape.${tgt_token_type} "
                fi
        ...
        if "${context}"; then
            if [ ${prev_context_tgt_type} = "rand" ]; then
                log "Adding random context to validation"
                _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
                _opts+="--valid_shape_file ${st_stats_dir}/valid/text_prev_rand_shape.${tgt_token_type} "

            elif [ ${prev_context_tgt_type} = "context2" ]; then
                log "Adding context 2 to validation"
                _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
                _opts+="--valid_shape_file ${st_stats_dir}/valid/text_prev2_shape.${tgt_token_type} "

            elif [ ${prev_context_tgt_type} = "context3" ]; then
                log "Adding context 3 to validation"
                _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
                _opts+="--valid_shape_file ${st_stats_dir}/valid/text_prev3_shape.${tgt_token_type} "

            else
                log "Adding previous context to validation"
                _opts+="--valid_data_path_and_name_and_type ${_st_valid_dir}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
                _opts+="--valid_shape_file ${st_stats_dir}/valid/text_prev_shape.${tgt_token_type} "
            fi
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.stcx_train \
				...
				--text_prev_apply_prob "${text_prev_apply_prob}" \
                --output_dir "${st_exp}" \
                ${_opts} ${st_args}
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_streaming}"; then
                ...
                st_inference_tool="espnet2.bin.stcx_inference"
            fi
            ...
            # shellcheck disable=SC2046,SC2086
            #_opts+="--data_path_and_name_and_type ${_data}/${_scp},speech,${_type} "
            #_opts+="--data_path_and_name_and_type ${_data}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
            if ! "${context}"; then
                    log "Decoding without context ..."
                ...
                if [ ${prev_context_tgt_type} = "rand" ]; then
                    log "Decoding with random context ..."
                    _opts+="--data_path_and_name_and_type ${_data}/text.prev.rand.${tgt_case}.${tgt_lang},text_prev,text "
                elif [ ${prev_context_tgt_type} = "context2" ]; then
                    log "Decoding with previous context 2 ..."
                    _opts+="--data_path_and_name_and_type ${_data}/text.prev2.${tgt_case}.${tgt_lang},text_prev,text "
                elif [ ${prev_context_tgt_type} = "context3" ]; then
                    log "Decoding with previous context 3 ..."
                    _opts+="--data_path_and_name_and_type ${_data}/text.prev3.${tgt_case}.${tgt_lang},text_prev,text "
                else
                    log "Decoding with previous context 1 ..."
                    _opts+="--data_path_and_name_and_type ${_data}/text.prev.${tgt_case}.${tgt_lang},text_prev,text "
                fi
